...
#include "lexer.h" /*operador[MAX_OPERADORES]*/
...
void mostreObjetoInfixa(CelObjeto *pObj, int tipoLista)
{
    if (pObj->categoria == FLOAT)
    {
        printf("%.0f ", pObj->valor.vFloat);
    }
    else
    {
...
mostreListaObjetos (CelObjeto *iniLista, int tipoLista)
{
    ...
    while (q != NULL)
    {
        if (tipoLista == POSFIXA)
        {
            if (p == NULL)
            {
                p = infixaParaPosfixa(iniLista);
                q = p->prox;
            }
            
            mostreObjetoInfixa(q, tipoLista);
        }
        else 
        {
            if (p == NULL)
